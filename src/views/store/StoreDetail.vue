...
<script >
  ...
  export default {
    ...
    methods: {
      ...
      downloadBook() {
        const opf = 'http://192.168.1.104:9000/epub' + `/${this.bookItem.categoryText}/${this.bookItem.fileName}/OEBPS/package.opf`
        this.parseBook(opf)
      },
      parseBook(url) {
        ...
        if (this.fileName) {
          detail({
            ...
            if (response.status === 200 && response.data.error_code === 0 && response.data.data) {
              ...
              this.opf = 'http://192.168.1.104:9000/epub2' + `/${this.fileName}/${rootFile}`
